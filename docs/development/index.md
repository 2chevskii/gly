...
CI runs both benchmark suites on `ubuntu-26.04`, publishes their median timings in the workflow summary, and stores the JSON results as the `benchmark-results` artifact. Each run compares matching scenarios with the latest successful `master` push. A scenario fails the regression gate when its median time is more than 20% slower; the first run and newly added scenarios establish a baseline instead.
